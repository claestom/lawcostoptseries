...
function Connect-WorkspaceToCluster {
    ...
    # Retry logic for workspace linking
    $maxRetries = 3
    $retryDelay = 30
    
    for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
        Write-Log "Attempt $attempt of $maxRetries for workspace: $workspaceName"
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Log "Attempt $attempt failed: $errorMsg" "WARNING"
            
            # Check for specific errors that indicate we should retry
            if ($errorMsg -match "InternalServerError|ServiceUnavailable|TooManyRequests" -and $attempt -lt $maxRetries) {
                Write-Log "Retrying in $retryDelay seconds..." "WARNING"
                Start-Sleep -Seconds $retryDelay
                $retryDelay *= 2  # Exponential backoff
            } else {
                Write-Log "Failed to link workspace: $workspaceName - $errorMsg" "ERROR"
                return $false
            }
        }
    }
    
    Write-Log "All retry attempts failed for workspace: $workspaceName" "ERROR"
    return $false
}
...
try {
    ...
    # Additional wait after cluster is ready - clusters need time to stabilize
    Write-Log "Waiting 60 seconds for cluster to stabilize before linking workspaces..."
    ...
    Write-Log "Process completed successfully! It can take up to 90 minutes before the workspaces are fully linked and displayed in the portal."
    Write-Log "To check link status, run following command: az monitor log-analytics workspace show --resource-group "resource-group-name" --workspace-name "workspace-name""
}
catch {
